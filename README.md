...
```
5. This step is needed for Windows. In your project, after you created cmake(example in dir documentation) you have to run these commands
```bash
cmake -S . -B build -DCMAKE_PREFIX_PATH="absolute/path/to/Os-project/install"
cmake --build build --config Release
```
...
2. Shared Memory(SharedMemoryChannel)\
...
3. Shared Semaphore(SharedSemaphore\
...
4. Socket Channel(SocketChannel)\
